...
void main() {
  ...
  group('syncDiff', () {
    ...
    test('removes presence when meta is empty and adds additional meta', () {
      ...
      expect(stateToMap(state), equals({
        'u1': {
          'metas': [
            {'id': 1, 'phx_ref': '1'},
            {'id': 1, 'phx_ref': '1.2'},
  ...
  group('list', () {
    test('lists full presence by default', () {
      ...
      final result = PhoenixPresence.listPresences(state, (key, entry) => entry);
      expect(result.map((e) => {'metas': e.metas}).toList(), equals([
  ...
  group('instance', () {
    ...
    test('syncs state and diffs', () {
      ...
      final user1 = (metas: [{'id': 1, 'phx_ref': '1'}] as List<Map<String, dynamic>>);
      final user2 = (metas: [{'id': 2, 'phx_ref': '2'}] as List<Map<String, dynamic>>);

      channel.trigger('presence_state', encodeState({'u1': user1, 'u2': user2}));
    ...
    test('applies pending diff if state is not yet synced', () {
      ...
      final user1 = (metas: [{'id': 1, 'phx_ref': '1'}] as List<Map<String, dynamic>>);
      final user2 = (metas: [{'id': 2, 'phx_ref': '2'}] as List<Map<String, dynamic>>);
      final user3 = (metas: [{'id': 3, 'phx_ref': '3'}] as List<Map<String, dynamic>>);
      ...
      expect(presence.list((k, e) => e.metas.first), equals([{'id': 1, 'phx_ref': '1'}]));
      ...
      channel.trigger('presence_state', encodeState({'u1': user1, 'u3': user3}));
      expect(presence.list((k, e) => e.metas.first), equals([{'id': 3, 'phx_ref': '3'}]));
    });
    ...
    test('updates existing meta for a presence update (leave + join)', () {
      ...
      final user1 = (metas: [{'id': 1, 'phx_ref': '1'}] as List<Map<String, dynamic>>);
      final user2 = (metas: [{'id': 2, 'name': 'chris', 'phx_ref': '2'}] as List<Map<String, dynamic>>);
    ...
    test('pending diffs flushed once each — onSync fires once per state event', () {
      var syncCount = 0;
      final presence = PhoenixPresence(channel: channel)
        ..onSync = () => syncCount++;
      ...
      expect(syncCount, 1); // one sync after state + both pending diffs applied
    ...
    test('multi-device: same user on two devices — metas merged correctly', () {
      ...
      final device1 = (metas: [{'phx_ref': 'ref_a', 'device': 'phone'}] as List<Map<String, dynamic>>);
      final device2 = (metas: [{'phx_ref': 'ref_b', 'device': 'laptop'}] as List<Map<String, dynamic>>);
      ...
      expect(metas.map((m) => m['phx_ref']).toSet(), equals({'ref_a', 'ref_b'}));
