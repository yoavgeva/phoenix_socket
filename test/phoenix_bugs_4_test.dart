...
class T {
  ...
  dynamic joinRefRaw(dynamic raw) =>
      (jsonDecode(raw as String) as List)[0];
...
void main() {
  ...
  group('Bug A: rejoin timeout clears push buffer completers', () {
    test('1. push buffered during joining state — rejoin times out — push errors promptly',
        () {
      fakeAsync((async) {
        final t = T()..init();
        ...
        Object? pushError;
        var pushDone = false;
        unawaited(ch.push('buffered_event', {}).then(
          (_) { pushDone = true; },
          onError: (e) { pushError = e; pushDone = true; },
        ));
        async.flushMicrotasks();
        ...
        async
          ..flushMicrotasks()
          ..flushMicrotasks();
        expect(pushDone, isTrue); // cleared by _clearPendingRefs on disconnect
        pushDone = false;
        pushError = null;
        ...
        unawaited(ch.push('during_rejoin', {}).then(
          (_) { pushDone = true; },
          onError: (e) { pushError = e; pushDone = true; },
        ));
        async.flushMicrotasks();
  ...
  group('Bug B: push after intentional disconnect throws immediately', () {
    test('3. push() after socket.disconnect() → StateError immediately', () async {
      final t = T()..init();
      await t.connect();
    ...
    test('4. intentional disconnect clears pending refs immediately', () async {
      ...
      // Push in flight
      Object? pushError;
      unawaited(ch.push('slow', {}).then((_) {}, onError: (e) { pushError = e; }));
      await flush(4);
    ...
    test('5. unintentional drop → channel errored (auto-rejoin eligible)', () async {
      final t = T()..init();
      await t.connect();
    ...
    test('6. join() after intentional disconnect → StateError (channel closed)', () async {
      final t = T()..init();
      await t.connect();
  ...
  group('Concurrent join() calls', () {
    test('7. two join() calls in same sync turn — second throws StateError', () async {
      final t = T()..init();
      await t.connect();
      ...
      Object? secondError;
      final f2 = ch.join().then((_) {}, onError: (e) { secondError = e; });
  ...
  group('Push timeout cleans up _pendingRefs', () {
    test('10. push times out — late reply is silently dropped (no double-complete)', () {
      fakeAsync((async) {
        final t = T()..init();
        unawaited(t.socket.connect().then((_) {}, onError: (_) {}));
        async
        ...
        Object? pushError;
        var pushDone = false;
        unawaited(ch.push('slow_ev', {}).then(
          (_) { pushDone = true; },
          onError: (e) { pushError = e; pushDone = true; },
        ));
        async.flushMicrotasks();
        ...
        final pushRef =
            t.ref(t.sent.where((m) => t.evnt(m) == 'slow_ev').last);
        ...
        t.toClient([jr, pushRef, 'room:push_timeout', 'phx_reply',
  ...
  group('_handleDisconnect idempotency', () {
    test('11. _onError + _onDone both fire — channels notified exactly once', () async {
      final t = T()..init();
      await t.connect();
  ...
  group('Unknown ref in phx_reply', () {
    test('12. server pushes phx_reply with ref nobody sent — no crash', () async {
      final t = T()..init();
      await t.connect();
      ...
      t.toClient([jr, '99999', 'room:unknown_reply_ref', 'phx_reply',
          {'status': 'ok', 'response': {'pushed': true}}]);
      await flush(4);
    ...
    test('13. server-push phx_reply does not appear on messages stream', () async {
      final t = T()..init();
      await t.connect();
      ...
      t.toClient([jr, '12345', 'room:no_phx_reply_stream', 'phx_reply',
          {'status': 'ok', 'response': <String, dynamic>{}}]);
      await flush(4);
  ...
  group('socket.channel() returns dead instance after leave()', () {
    test('14. channel() after leave() → same instance → join() throws StateError', () async {
      final t = T()..init();
      await t.connect();
  ...
  group('Push buffer survives reconnect → rejoin → flush', () {
    test('15. push buffered during joining state → rejoin succeeds → push resolves', () {
      fakeAsync((async) {
        final t = T()..init();
        unawaited(t.socket.connect().then((_) {}, onError: (_) {}));
        async
        ...
        var pushDone = false;
        unawaited(ch.push('queued_while_joining', {'v': 99}).then(
          (res) { pushResult = res; pushDone = true; },
          onError: (_) { pushDone = true; }, // drop() error-completes this
        ));
        async.flushMicrotasks();
        ...
        pushDone = false;
        pushResult = {};
        unawaited(ch.push('after_rejoin', {'v': 42}).then((res) {
          pushResult = res;
          pushDone = true;
        }));
        async.flushMicrotasks();
        ...
        t.toClient([pjr, pr, 'room:buf_rejoin', 'phx_reply',
