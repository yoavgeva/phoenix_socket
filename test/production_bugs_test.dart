...
class S {
  ...
  void replyOk(String joinRef, String msgRef, String topic,
      [Map<String, dynamic> resp = const {}]) {
    toClient([joinRef, msgRef, topic, 'phx_reply',
      {'status': 'ok', 'response': resp}]);
  }

  void replyErr(String joinRef, String msgRef, String topic,
      [Map<String, dynamic> resp = const {}]) {
    toClient([joinRef, msgRef, topic, 'phx_reply',
...
void main() {
  ...
  group('Continuous rejoin loop prevention', () {
    ...
    test('2. repeated server crashes — rejoin count matches reconnect count', () {
      fakeAsync((async) {
        final s = S()..init();
        unawaited(s.socket.connect());
        async
  ...
  group('No send after close', () {
    ...
    test('6. heartbeat timer cancelled on disconnect — no send after close', () {
      fakeAsync((async) {
        final s = S()..init();
        unawaited(s.socket.connect());
        async
    ...
    test('7. channel push after disconnect does not throw — state guard', () async {
      final s = S()..init();
      await s.connect();
  ...
  group('State always reflects reality', () {
    test('8. connection refused → state becomes reconnecting, never stuck connecting',
        () {
      fakeAsync((async) {
        final socket = PhoenixSocket(
    ...
    test('9. socket state stream always reflects internal state', () async {
      ...
      final disconnectedIdx = states.lastIndexOf(PhoenixSocketState.disconnected);
  ...
  group('Push ref cleanup', () {
    ...
    test('11. disconnect clears all pending refs — no zombie completers', () async {
      final s = S()..init();
      await s.connect();
  ...
  group('Leave during joining', () {
    test('12. leave() while joining — join future completes with error', () async {
      final s = S()..init();
      await s.connect();
    ...
    test('13. leave() while joining — leave() completes even with no server reply',
        () async {
      final s = S()..init();
      await s.connect();
  ...
  group('Heartbeat boundary race', () {
    test('14. heartbeat reply arrives just before second tick — no reconnect', () {
      fakeAsync((async) {
        final s = S()..init();
        unawaited(s.socket.connect());
        async
        ...
        async
          ...
          ..flushMicrotasks();
        s.toClient([null, hbRef, 'phoenix', 'phx_reply',
    ...
    test('15. heartbeat reply arrives 1ms after second tick — reconnect fires', () {
      fakeAsync((async) {
        final s = S()..init();
        unawaited(s.socket.connect());
        async
  ...
  group('Idempotent disconnect', () {
    test('16. disconnect() called 3 times — no crash, state stays disconnected',
        () async {
      final s = S()..init();
      await s.connect();
      await s.socket.disconnect();
    ...
    test('17. disconnect() then connect() then disconnect() — clean cycle',
        () async {
      final s = S()..init();
      await s.connect();
      await s.socket.disconnect();
  ...
  group('Channel topic reuse after close', () {
    test('18. socket routes messages only to tracked channels — not to replaced ones',
        () async {
      final s = S()..init();
      await s.connect();
      ...
      s.toClient([null, null, 'room:other', 'event', {'x': 1}]);
      await flush(4);
      expect(events, isEmpty);

      // Message for correct topic — must reach ch
      s.toClient([null, null, 'room:routing', 'my_event', {'x': 2}]);
      await flush(4);
    ...
    test('19. old push reply with stale joinRef not routed to new channel',
        () async {
      final s = S()..init();
      await s.connect();
  ...
  group('Multiple sockets independence', () {
    test('20. two sockets same URL — disconnect one does not affect other',
        () async {
      final s1 = S()..init();
      final s2 = S()..init();
  ...
  group('Rejoin payload immutability', () {
    ...
    test('23. mutating original payload map after join does not affect rejoin',
        () {
      fakeAsync((async) {
        final s = S()..init();
        unawaited(s.socket.connect());
  ...
  group('Malformed server replies', () {
    test('24. phx_reply with null status — handled gracefully', () async {
      ...
      fakeAsync((async) {
        unawaited(ch.push('action', {}).then((_) {},
        ...
        s.toClient([joinRef, pushRef, 'room:null-status', 'phx_reply',
    ...
    test('25. phx_reply with missing response field — defaults to empty map',
        () async {
      final s = S()..init();
      await s.connect();
      ...
      fakeAsync((async) {
        unawaited(ch.push('action', {}).then((r) { result = r; }));
        async.flushMicrotasks();
        final pushRef = s.ref(s.sent.lastWhere((r) => s.event(r) == 'action'));
        final joinRef = s.jRef(s.sent.lastWhere((r) => s.event(r) == 'action'));

        // Reply without 'response' field
        s.toClient([joinRef, pushRef, 'room:missing-resp', 'phx_reply',
          {'status': 'ok'}]);
        async
          ..flushMicrotasks()
          ..flushMicrotasks();
    ...
    test('26. phx_reply with extra unexpected fields — ignored cleanly', () async {
      final s = S()..init();
      await s.connect();
      ...
      var done = false;
      fakeAsync((async) {
        unawaited(ch.push('action', {}).then((_) { done = true; }));
        async.flushMicrotasks();
        final pushRef = s.ref(s.sent.lastWhere((r) => s.event(r) == 'action'));
        final joinRef = s.jRef(s.sent.lastWhere((r) => s.event(r) == 'action'));

        // Reply with extra fields
        s.toClient([joinRef, pushRef, 'room:extra-fields', 'phx_reply',
          {'status': 'ok', 'response': {'data': 42}, 'unknown': 'ignored'}]);
        async
          ..flushMicrotasks()
          ..flushMicrotasks();
        expect(done, isTrue);
  ...
  group('Orphaned channel state', () {
    test('27. channel in joining state when socket crashes — ends up errored',
        () async {
      final s = S()..init();
      await s.connect();
    ...
    test('28. buffered push while joining errors when socket crashes', () async {
      final s = S()..init();
      await s.connect();
  ...
  group('Push buffer in errored state', () {
    test('29. pushes buffered during errored state are sent after rejoin',
      ...
      fakeAsync((async) {
        ...
        unawaited(ch.push('buffered_event', {'x': 1}).then((_) {}, onError: (_) {}));
        async
          ..flushMicrotasks()
          ..elapse(const Duration(seconds: 2))
          ..flushMicrotasks();
    ...
    test('30. pushes buffered during errored state error on disconnect', () async {
      final s = S()..init();
      await s.connect();
      ...
      unawaited(ch.push('buffered', {}).then((_) {},
          onError: (_) { bufferedErrored = true; }));
      await flush();
  ...
  group('Concurrent connect() calls', () {
    ...
    test('33. connect() during connecting state — second call is no-op', () async {
      final s = S()..init();
  ...
  group('Control event filtering (regression)', () {
    ...
    test('35. all control events filtered from messages stream', () async {
      ...
      for (final ctrl in ['phx_reply', 'phx_join', 'phx_leave', 'phx_close',
          'phx_error']) {
    ...
    test('36. app events DO appear on messages stream', () async {
      ...
      s
        ..toClient([null, null, 'room:app-events', 'new_message', {'text': 'hi'}])
        ..toClient([null, null, 'room:app-events', 'user_joined', {'id': 1}]);
      await flush(8);

      expect(msgs.map((m) => m.event).toList(),
  ...
  group('Send while disconnected', () {
    test('39. send() while disconnected — no exception thrown, no crash',
        () async {
      final s = S()..init();
      // Not connected — send should be silently ignored
      s.socket.send(const PhoenixMessage(
        joinRef: null,
        ref: '1',
        topic: 'room:test',
        event: 'test',
    ...
    test('40. push() while connected → disconnect fires mid-push — push errors',
        () async {
      final s = S()..init();
      await s.connect();
