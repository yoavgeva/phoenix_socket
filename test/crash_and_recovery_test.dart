...
void main() {
  ...
  group('Server crash', () {
    ...
    test('2. server crash during active push — push errors, reconnect fires',
        () async {
      final s = CrashSetup()..init();
      await s.connect();
    ...
    test('4. server crash + rejoin: channel re-sends phx_join after reconnect',
        () {
      fakeAsync((async) {
        final s = CrashSetup()..init();
        unawaited(s.socket.connect());
        ...
        final rejoinMsgs = s.sent
            ...
            .toList();
        expect(rejoinMsgs.length, greaterThanOrEqualTo(2)); // original + rejoin
    ...
    test('5. server restarts repeatedly — socket survives N crash+reconnect cycles',
        () {
      fakeAsync((async) {
        var connects = 0;
    ...
    test('6. server crash clears all pending push completers', () async {
      ...
      final errors = <Object>[];
      for (var i = 0; i < 10; i++) {
        unawaited(ch.push('event', {'i': i}).catchError((Object e) {
          errors.add(e);
          return <String, dynamic>{};
        }));
      }
  ...
  group('App backgrounding and resume', () {
    test('8. app backgrounded for 60s — heartbeat missed — socket reconnects',
        () {
      fakeAsync((async) {
        final s = CrashSetup()..init();
        unawaited(s.socket.connect());
        ...
        expect(
          s.socket.state,
          isIn([PhoenixSocketState.reconnecting, PhoenixSocketState.connected]),
        );
    ...
    test('9. app backgrounded 5 minutes — eventually reconnects when resumed',
        () {
      fakeAsync((async) {
        final s = CrashSetup()..init();
        unawaited(s.socket.connect());
    ...
    test(
        '10. channel state after background+reconnect: errored channels '
    ...
    test(
        '11. push buffered during background reconnect is sent after '
      ...
      unawaited(ch.push('background_event', {'data': 'queued'})
          .then((r) => pushResult = r));
      await _flush();
      ...
      s.sendToClient([
        ...
        'room:lobby',
        'phx_reply',
        {'status': 'ok', 'response': {'received': true}}
      ]);
      await _flush(8);
  ...
  group('Network flapping', () {
    ...
    test(
        '13. rapid flapping: connect → crash → connect → crash × 3 '
    ...
    test(
        '14. channel accumulates push buffer during repeated flaps '
    ...
    test('15. socket disconnect/reconnect does not lose message listener', () async {
      final s = CrashSetup()..init();
      await s.connect();
      ...
      s.sendToClient([ref, null, 'room:live', 'update', {'x': 7}]);
      await _flush();
  ...
  group('Race conditions', () {
    test(
        '16. disconnect() called while reconnect timer is in-flight — '
        'timer is cancelled', () {
      fakeAsync((async) {
        var connects = 0;
        final socket = PhoenixSocket(
          ...
          channelFactory: (uri) {
            ...
            if (connects == 1) unawaited(ws.server.sink.close()); // drop first connection
            return ws;
          },
        );
    ...
    test('17. simultaneous join + server error → no dangling completer', () async {
      final s = CrashSetup()..init();
      await s.connect();
      final ch = s.socket.channel('room:lobby');
      ...
      s
        ...
        ..sendToClient([ref, null, 'room:lobby', 'phx_error', <String, dynamic>{}]);
      await _flush();
      await joinF;
    ...
    test(
        '18. push sent, reply arrives, then disconnect — no double error', () async {
      final s = CrashSetup()..init();
      await s.connect();
      final ch = await s.joinedChannel('room:lobby');
      ...
      var succeeded = false;
      var failed = false;
      unawaited(ch.push('event', {}).then((_) {
        succeeded = true;
      }, onError: (_) {
        failed = true;
      }));
    ...
    test(
        '19. leave() called right as join() reply arrives — '
    ...
    test(
        '21. channel.rejoin() called while already in joining state — '
    ...
    test(
        '22. push reply arrives after channel was left — '
      ...
      unawaited(ch.push('event', {}).catchError((Object e) {
        pushErrored = true;
        return <String, dynamic>{};
      }));
      await _flush();
  ...
  group('App crash simulation', () {
    ...
    test('27. channel.leave() on a socket that is already disconnected', () async {
      final s = CrashSetup()..init();
      await s.connect();
      final ch = await s.joinedChannel('room:lobby');
  ...
  // =========================================================================
  group('Ref counter', () {
    test('29. ref counter increments monotonically across many operations',
        () async {
      final s = CrashSetup()..init();
      await s.connect();
      final ch = await s.joinedChannel('room:lobby');
  ...
  group('Duplicate and weird server replies', () {
    ...
    test('33. server sends push reply with unknown ref — silently ignored',
        () async {
      final s = CrashSetup()..init();
      await s.connect();
      final ch = await s.joinedChannel('room:lobby');
    ...
    test('34. server sends phx_close then immediately sends another message',
        () async {
      final s = CrashSetup()..init();
      await s.connect();
      final ch = await s.joinedChannel('room:lobby');
      ...
      s
        ..sendToClient([joinRef, null, 'room:lobby', 'phx_close', <String, dynamic>{}])
        ..sendToClient([joinRef, null, 'room:lobby', 'broadcast', {'x': 1}]);
      await _flush();
    ...
    test(
      ...
      // First phx_error
      s.sendToClient([joinRef, null, 'room:lobby', 'phx_error', <String, dynamic>{}]);
      await _flush();
      expect(ch.state, PhoenixChannelState.errored);
  ...
  group('Listener lifecycle', () {
    test('38. listener added after first messages — only receives subsequent',
        () async {
      final s = CrashSetup()..init();
      await s.connect();
      final ch = await s.joinedChannel('room:lobby');
      ...
      s.sendToClient([joinRef, null, 'room:lobby', 'early', {'n': 1}]);
      await _flush();
      ...
      s.sendToClient([joinRef, null, 'room:lobby', 'late', {'n': 2}]);
      await _flush();
    ...
    test('39. two listeners on messages stream: both receive events', () async {
      ...
      s.sendToClient([joinRef, null, 'room:lobby', 'event', <String, dynamic>{}]);
      await _flush();
    ...
    test('40. socket states stream broadcast: listener added after connect',
        () async {
      final s = CrashSetup()..init();
      await s.connect();
    ...
    test(
        '42. channel messages stream can have many listeners without '
      ...
      s.sendToClient([joinRef, null, 'room:lobby', 'multicast', <String, dynamic>{}]);
      await _flush(6);
  ...
  group('Graceful shutdown under load', () {
    test('43. disconnect() while 50 channels are joining', () async {
      ...
      final errors = <Object>[];
      for (var i = 0; i < 50; i++) {
        unawaited(s.socket.channel('room:$i').join().catchError((Object e) {
          errors.add(e);
          return <String, dynamic>{};
        }));
      }
    ...
    test(
      ...
      final errors = <Object>[];
      for (var i = 0; i < 7; i++) {
        unawaited(chA.push('event', {}).catchError((Object e) {
          errors.add(e);
          return <String, dynamic>{};
        }));
        unawaited(chB.push('event', {}).catchError((Object e) {
          errors.add(e);
          return <String, dynamic>{};
        }));
      }
      for (var i = 0; i < 6; i++) {
        unawaited(chC.push('event', {}).catchError((Object e) {
          errors.add(e);
          return <String, dynamic>{};
        }));
      }
  ...
  group('Protocol invariants under stress', () {
    test(
        '46. all push completers cleaned up after disconnect — '
    ...
    test('48. messages from other topics do not interfere with joined channels',
        () async {
      final s = CrashSetup()..init();
      await s.connect();
      final ch = await s.joinedChannel('room:lobby');
      ...
      s
        ..sendToClient([null, null, 'system:alerts', 'alert', {'level': 'warn'}])
        ..sendToClient([joinRef, null, 'room:lobby', 'update', {'v': 1}]);
      await _flush(6);
    ...
    test('49. push sent right at 10s boundary — either resolves or times out',
        () {
      fakeAsync((async) {
        final s = CrashSetup()..init();
        unawaited(s.socket.connect());
    ...
    test(
        '50. end-to-end: server crash → reconnect → rejoin → push → '
