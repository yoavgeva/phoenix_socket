...
void main() {
  ...
  group('syncDiff: leave for unknown phx_ref', () {
    test('stale ref in leave payload does not remove any meta', () {
      ...
      final result = PhoenixPresence.syncDiff(
        state,
        joins: {},
        leaves: {'u1': entry([meta('stale_ref')])},
      );
  ...
  group('syncDiff: join and leave same key', () {
    test('join processed before leave — net result is new meta only', () {
      ...
      final state = {
        'u1': entry([meta('ref_old', {'v': 1})])
      };
      final result = PhoenixPresence.syncDiff(
        state,
        joins: {'u1': entry([meta('ref_new', {'v': 2})])},
    ...
    test('onJoin fires before onLeave for same key', () {
      final events = <String>[];
      final state = {'u1': entry([meta('old')])};
      PhoenixPresence.syncDiff(
        state,
        joins: {'u1': entry([meta('new')])},
        leaves: {'u1': entry([meta('old')])},
  ...
  group('syncState: unchanged key triggers no callbacks', () {
    test('no onJoin and no onLeave when metas are identical', () {
      final state = {'u1': entry([meta('ref1')])};
      final newState = {'u1': entry([meta('ref1')])};
  ...
  group('syncState: partial meta overlap', () {
    test('only genuinely new refs trigger onJoin; only gone refs trigger onLeave', () {
  ...
  group('callback argument independence', () {
    test('mutating newPres inside onJoin does not affect stored state', () {
      ...
      PhoenixPresence.syncDiff(
        state,
        joins: {'u1': entry([meta('1', {'data': 'original'})])},
        leaves: {},
        onJoin: (_, _, newPres) {
      ...
      final result = PhoenixPresence.syncDiff(
        {'u1': entry([meta('1', {'data': 'original'})])},
        joins: {'u1': entry([meta('2')])},
        leaves: {},
      );
  ...
  group('instance: pending diff queue ordering', () {
    ...
    test('pending diffs cleared after snapshot — subsequent diff not queued', () {
      final presence = PhoenixPresence(channel: channel);
      final u1 = entry([meta('1')]);
  ...
  group('instance: reconnect discards old pending diffs', () {
    ...
    test('diffs queued before reconnect are flushed when NEW state arrives', () {
      final presence = PhoenixPresence(channel: channel);
      final u1 = entry([meta('1')]);
      final u2 = entry([meta('2')]);
  ...
  group('instance: onSync call count', () {
    ...
    test('onSync not called for diffs received while pending', () {
      ...
      channel
        ..trigger('presence_diff', encodeDiff(joins: {'u1': entry([meta('1')])}))
        ..trigger('presence_diff', encodeDiff(joins: {'u2': entry([meta('2')])}));
    ...
    test('each non-pending diff triggers its own onSync', () {
      ...
      channel.trigger('presence_diff', encodeDiff(joins: {'u1': entry([meta('1')])}));
      expect(syncCount, 2);

      channel.trigger('presence_diff', encodeDiff(joins: {'u2': entry([meta('2')])}));
      expect(syncCount, 3);
    });
  });
  ...
  group('instance: edge payloads', () {
    ...
    test('user with many metas — all removed one by one', () {
      ...
      final metas = List.generate(
          5, (i) => meta('ref_$i', {'device': 'device_$i'}));
      channel.trigger('presence_state', encodeState({
        'u1': entry(metas),
      }));
  ...
  group('Bug: nested map in meta — shallow cast', () {
    ...
    test('nested map in presence_state meta is accessible as Map<String,dynamic>', () {
      final presence = PhoenixPresence(channel: channel);
      // Meta contains a nested object — common in real apps (location, device info, etc.)
      channel.trigger('presence_state', {
        'u1': {
          'metas': [
            {'phx_ref': '1', 'location': {'lat': 51.5, 'lng': -0.1}, 'tags': ['vip']}
          ]
        }
      });
      ...
      channel
        ...
        ..trigger('presence_diff', {
          'joins': {
            'u1': {
              'metas': [
                {'phx_ref': '1', 'device': {'os': 'iOS', 'version': 17}}
              ]
            }
          },
  ...
  group('Bug: zombie key from empty metas in join', () {
    ...
    test('syncDiff: join with empty metas for new key does not create zombie entry', () {
      final result = PhoenixPresence.syncDiff(
        {},
        joins: {'u1': entry([])},
        leaves: {},
    ...
    test('syncState: new key with empty metas in server snapshot is not stored', () {
      final result = PhoenixPresence.syncState(
        {},
        {'u1': entry([])},
      );
    ...
    test('instance: presence_state with empty metas does not appear in list()', () {
      final presence = PhoenixPresence(channel: channel);
      channel.trigger('presence_state', {
        'u1': {'metas': []},
        'u2': {'metas': [{'phx_ref': '2'}]},
    ...
    test('instance: presence_diff join with empty metas does not appear in list()', () {
      final presence = PhoenixPresence(channel: channel);
      channel
        ..trigger('presence_state', encodeState({}))
        ..trigger('presence_diff', {
  ...
  group('instance: duplicate presence_state on same connection', () {
    ...
    test('second presence_state does not re-apply already-flushed pending diffs', () {
      final presence = PhoenixPresence(channel: channel);
      var joinCount = 0;
